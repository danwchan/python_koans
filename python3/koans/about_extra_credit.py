...
#        player1 = GreedPlayer('player_1')

#        self.assertEquals(player1._outcome, "undecided")

#        player1.create_player("Daniel Winston Chan")

#        self.assertEquals(player1.name, "Daniel Winston Chan")

    @patch('builtins.input', side_effect=['not a name', 'n', 'Daniel', 'y', 'Winston', 'y', 'Chan', 'n', 'Chantinez' ,'n', 'Danny', 'y'])
    def test_game_can_create_players(self, mock_input):
        game1 = GreedGame()
        game1.initialize_players(3)
        
        self.assertEquals(game1._num_players, 3)

        playerlist = list()
        for num in range(game1._num_players):
            playerlist.append(next(game1._players))

        self.assertEquals(playerlist[1].name, 'Winston')
        self.assertEquals(playerlist[2]._id, 'player_3')
        self.assertEquals(playerlist[2].in_the_game, False)

    def test_scoring_only_after_300_pts(self, mock_input):
        pass
